const {db} = require('../configs/db/postgre-connection');
const {logger} = require('../configs/winstonLogger/winstonLogger');



async function getAll(){
    let sql = 'SELECT * FROM discount'
    let result;
    try {
        result = await db.any(sql);
    } catch (error) {
        logger.error(`getAll failed.`,{"file":__filename,message:error.message});
    }
    return result;
  
}

async function getById(id){
    let sql = 'SELECT * FROM discount WHERE id = $1'
    let result;
    try {
        result = await db.oneOrNone(sql,id); 
    } catch (error) {
        logger.error(`getById failed.`,{"file":__filename,message:error.message});
    }
    return  result;
}


async function create(body){
    let sql = "INSERT INTO discount(title, descript, discount_percent, is_active) VALUES($1,$2,$3,$4) RETURNING *";
    let values = [body.title, body.descript, body.discount_percent, body.is_active];
    let result;
    try {
        result = await db.oneOrNone(sql, values);
    } catch (error) {
        logger.error(`create failed.`,{"file":__filename,message:error.message});
    }
    return result;
}

async function updateById(body){
    let sql = 'UPDATE discount SET title = $1, descript = $2, discount_percent = $3, is_active = $4 WHERE id = $5 RETURNING *';
    let values = [body.title, body.descript, body.discount_percent, body.is_active, body.id];
    let result;
    try {
        result = await db.oneOrNone(sql,values);
    } catch (error) {
        logger.error(`updateById failed.`,{"file":__filename,message:error.message});
    }
    return result;
}

async function deleteById(body){
    let sql = 'DELETE FROM discount WHERE id = $1 RETURNING *';
    let result;
    try {
        result = await db.oneOrNone(sql,body.id);
    } catch (error) {
        logger.error(`deleteById failed.`,{"file":__filename,message:error.message});
    }
    return result;
}


module.exports = {
    create,
    getAll,
    getById,
    updateById,
    deleteById
}